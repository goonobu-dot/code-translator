...
- 必要なもの: Python 3 / Git / [Claude Code CLI](https://claude.com/claude-code)(翻訳の生成に使用)
- **初回は翻訳が終わるまで待つ**(画面は完了後に自動で開く)。目安は**1,000行あたり約1.5分**(実測): 200行→約2分、3,000行→約5分、8,000行→約12分。実行中は`[7/35] ファイル名(残り約4.2分)`と進捗表示
- 翻訳はClaude Codeの利用枠内で動くため**追加請求なし**(APIキー直接利用の場合のみAPI利用分がかかる)
- コードに変更がなければ再翻訳せずスキップ
